...
import { X, Moon, Palette, Sliders, Check } from 'lucide-react'; 
...
const SettingsModal = ({ isOpen, onClose }) => {
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to restore original settings?")) {
      const defaultData = {
          darkMode: false,
          accentColor: 'Blue',
          displayName: 'John Doe',
          jobTitle: 'Team Group 11' 
      };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl max-h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-up transition-colors duration-300">
              ...
              <div className="space-y-3">
                ...
                <div className="border border-gray-200 dark:border-slate-700 rounded-xl p-4 flex items-center justify-between bg-white dark:bg-slate-800 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-gray-100 dark:bg-slate-700 rounded-lg"><Moon size={20} className="text-gray-600 dark:text-blue-400" /></div>
                    <div><p className="font-medium text-gray-900 dark:text-white">Dark Mode</p></div>
                  </div>
                  <button 
                  ...
                  >
                    <span className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full shadow-sm transform transition-transform duration-300 ${formData.darkMode ? 'translate-x-6' : 'translate-x-0'}`} />
                  </button>
                </div>
                ...
                <div className="border border-gray-200 dark:border-slate-700 rounded-xl p-4 flex items-center justify-between mt-3 bg-white dark:bg-slate-800 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-gray-100 dark:bg-slate-700 rounded-lg"><Palette size={20} className="text-gray-600 dark:text-blue-400" /></div>
                    <div><p className="font-medium text-gray-900 dark:text-white">Accent Color</p></div>
                  </div>
                  <div className="flex gap-2">
                    {COLORS.map((color) => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${color.class} ${formData.accentColor === color.name ? 'ring-2 ring-offset-2 scale-110' : 'opacity-70'}`}
                      >
                        {formData.accentColor === color.name && <Check size={14} className="text-white" strokeWidth={3} />}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full border border-gray-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg px-4 py-2.5 outline-none focus:border-blue-500 transition-all"
                    placeholder="Enter your name"
                  />
                </div>

                {/* 2. Job Title / Project Name Input */}
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Job Title / Team Name</label>
                  <input 
                    type="text" 
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-slate-700 shrink-0 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900 transition-colors">
          <button onClick={handleReset} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 border border-gray-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
            Reset to Default
          </button>
          <div className="flex gap-3">
             <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors">Cancel</button>
            <button onClick={handleSave} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm shadow-blue-200 dark:shadow-none transition-colors">Save Changes</button>
          </div>
        </div>
