...
class ProjectService {
    ...
    async addMemberToProject(projectId, userId, role, requesterId) {
        ...
        return await projectMembersRepository.addMember(
            projectId,
            userId,
            role
        );
    ...
    async removeMemberFromProject(projectId, userId, requesterId) {
        ...
        if (requesterRole !== "manager") {
            throw new Error(
                "Only managers can remove members from the project"
            );
        }
    ...
    async getUserProjects(userId) {
        const projectMembers =
            await projectMembersRepository.findProjectsByUser(userId);
        ...
        const result = await Promise.all(
            projectMembers.map(async (pm) => {
                ...
                const members =
                    await projectMembersRepository.findMembersByProject(
                        pm.project._id
                    );
                if (!project) return null;

                // Progress (done/total)
                const issues = await issueRepository.findByProjects(
                    project._id
                );
                const doneCount = issues.filter(
                    (issue) => issue.status === "done"
